...
elvis5.scene.SceneManager = function(param) {
    ...
    if(param.window_size) {
        ...
        if(param.renderer.container) {
            this.window_size = {
                width : param.renderer.container.clientWidth,
                height : param.renderer.container.clientHeight
            }
    ...
    switch(param.camera.type)
    {
      ...
      case 'ortho2d':
      {
        console.log('make ortho2d cam')

        let frustumSize = param.camera.frustumSize;
        let aspect = param.camera.aspect;

          this.camera = new THREE.OrthographicCamera( frustumSize * aspect / - 2, frustumSize * aspect / 2, frustumSize / 2, frustumSize / - 2,
          0.5, 1000 );

        param.camera.position = new THREE.Vector3(0,0,500)
        param.camera.lookat = new THREE.Vector3(0,0,0)
